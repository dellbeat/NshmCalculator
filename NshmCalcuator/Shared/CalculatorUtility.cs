...
public static class CalculatorUtility
{
    ...
    public static (double, double) CalculateIncreaseRate(PlayerBaseInfo baseInfo, double attack = 0, int breakDefense = 0, int elementAttack = 0, int restraint = 0, double hit = 0, double criticalHits = 0, double criticalRate = 0, bool fixHitMode = false)
    {
        ...
        double baseHit = fixHitMode ? baseInfo.PlayerBaseHit > baseInfo.FullHit ? baseInfo.FullHit : baseInfo.PlayerBaseHit : baseInfo.PlayerBaseHit;
        double addedHit = fixHitMode ? (baseInfo.PlayerBaseHit + hit > baseInfo.FullHit ? baseInfo.FullHit : baseInfo.PlayerBaseHit + hit) : baseInfo.PlayerBaseHit + hit;

        double b1 = (115 * (baseInfo.PlayerBaseCriticalHits - baseInfo.EnemyAntiCriticalHits) + 90) * 1.0 / (baseInfo.PlayerBaseCriticalHits - baseInfo.EnemyAntiCriticalHits + 940) / 100 + baseInfo.PlayerBaseZtCriticalHitsRate * 1.0 / 100;//暴击百分比
        double b2 = (95 + 143 * baseHit * 1.0 / (baseHit + 713) - 143 * baseInfo.EnemyBlock * 1.0 / (baseInfo.EnemyBlock + 713)) / 100;//命中值
        double b3 = (115 * (baseInfo.PlayerBaseCriticalHits - baseInfo.EnemyAntiCriticalHits + criticalHits) + 90) * 1.0 / (baseInfo.PlayerBaseCriticalHits - baseInfo.EnemyAntiCriticalHits + criticalHits + 940) / 100 + baseInfo.PlayerBaseZtCriticalHitsRate * 1.0 / 100;//新增后暴击百分比
        ...
        double b9 = (baseInfo.PlayerBaseAttack + baseInfo.PlayerBaseRestraint - baseInfo.EnemyAntiRestraint + 920) * 2860 * 1.0 / (2860 + defenseSubBase) + baseInfo.PlayerBaseElementAttack - baseInfo.EnemyAntiElementAttack;//等效元素攻击
        ...
        double b12 = ((baseInfo.PlayerBaseAttack + baseInfo.PlayerBaseRestraint - baseInfo.EnemyAntiRestraint + 920) * 2860 * 1.0 / (2860 + defenseSubBase) *
                      (2860 + defenseSubBase) / (2860 + defenseSubFull) + baseInfo.PlayerBaseElementAttack - baseInfo.EnemyAntiElementAttack) / b9 - 1;//破防收益率
    ...
    public static EarningRate CalculatePerGains(PlayerBaseInfo baseInfo)
    {
        EarningRate rate = new EarningRate
        {
            ...
            Hit = CalculateIncreaseRate(baseInfo, hit: 1, fixHitMode: true).Item1,
