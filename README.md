...
* PVE收益计算器（B站@<b>-风何往-</b>）：已实现原工具内部分数值收益计算的功能,后续计划实现内功整体收益/换装收益计算等功能（1/3）
* 身份收益计算器（B站@<b>逆水寒-夜银</b>）：已实现原工具内身份收益计算的功能（1/1）
* 素问治疗计算器（小红书<b>@师尹</b>）：已实现原工具内治疗量计算功能。（1/1）

## 已下架计算器归档
* 属性收益计算器（贴吧@<b>星语困/kkxp/瑶光听雪-欧皇新落</b>|已停更）：已实现原工具内期望伤害计算/属性变化伤害提升计算/属性提升收益曲线图；面板对比器暂无实现计划。（3/4）
* 增伤百分比计算器（B站<b>@白宝正不正</b>|已停更）：已实现原工具内增伤率计算功能。（1/1）
* 素问治疗量计算器（B站<b>@白宝正不正</b>|已停更）：已实现原工具内治疗量计算功能。（1/1）
* 内功收益计算器（NGA<b>@UID:42330896</b>|已停更）：已实现原工具内属性提升率计算/内功数据存储、计算/动态计算基础词条加成；内功特性增伤表、周天增伤率简表暂无实现计划。（3/5）

## 内容来源及鸣谢

B站@<b>-风何往-</b>：[-风何往-的个人空间](https://space.bilibili.com/396307349/dynamic) （PVE收益计算器）

B站@<b>逆水寒-夜银</b>：[对逆水寒手游新赛年赚铜思路的分享](https://www.bilibili.com/video/BV1HS411w7MJ) （身份收益计算器）

小红书<b>@师尹</b>：[素问疗强转换机制+最新版素问计算器](https://www.xiaohongshu.com/explore/6685277b0000000003024ba6) （素问治疗计算器）

[arkntools/arknights-toolbox](https://github.com/arkntools/arknights-toolbox) （设计思路参考）

B站<b>@清纯男大阿娟</b>：[清纯男大阿娟的个人空间](https://space.bilibili.com/11869478)（2.1首领数据参考）
...
* NshmCalculator.Test：该项目为单元测试项目，由于目前已转用动态公式配置的形式，用途已更改为校验配置文件格式是否有误
