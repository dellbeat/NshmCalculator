...
* 身份收益计算器（B站@<b>逆水寒-夜银</b>）：已实现元工具内
* 素问治疗计算器（小红书<b>@师尹</b>）：已实现原工具内治疗量计算功能。（1/1）
* 属性收益计算器（贴吧@<b>星语困/kkxp/瑶光听雪-欧皇新落</b>）：已实现原工具内期望伤害计算/属性变化伤害提升计算/属性提升收益曲线图；面板对比器暂无实现计划。（3/4）
* 增伤百分比计算器（已停更）：已实现原工具内增伤率计算功能。（1/1）
* 素问治疗量计算器（已停更）：已实现原工具内治疗量计算功能。（1/1）
* 内功收益计算器（已停更）：已实现原工具内属性提升率计算/内功数据存储、计算/动态计算基础词条加成；内功特性增伤表、周天增伤率简表暂无实现计划。（3/5）

## 部分计算器暂停更新说明

目前白宝/星语困/NGA等大佬的计算器已长期未更新；且目前正在尝试复刻的EXCEL计算器也将转为打赏使用模式。

我后续不能在没有特别授权且未对数据算法代码进行处理的前提下继续复刻该计算器了。

~~本在线计算器从现在开始处于无限期暂停更新的状态。~~

上述计算器目前已收纳至暂停更新区域。

如有类似命中格挡公式等的改动，且有可靠信源/算法等，我会在验证之后进行更新。

本在线计算器可正常使用，目前不会关站，至少会到明年的六月底，后续会根据网站统计系统的数据决定是否再继续开站。

最后很感谢大家从三月底以来的支持，刚去看了一眼到现在浏览量超过了1W+，算是个里程碑了。
